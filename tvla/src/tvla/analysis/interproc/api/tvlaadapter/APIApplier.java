...
public final class APIApplier {
  ...
  final private ActionApplier applier;
  ...
  public APIApplier(
      ...
      boolean breakIfCoerceAfterUpdateFailed, String marker) {
    super();
    this.voc = voc;
    ...
    applier = new ActionApplier (
        ...
        doCoerceAfterUpdate,
        doBlur,
        freezeStructuresWithMessages, breakIfCoerceAfterUpdateFailed);	
  ...
  public Collection apply(
    ...
    // DHACK 
    //TVLAAPITrace.tracePrintln("APIApplier " + marker + " counter = " + ++counter);
  ...
  public class ActionApplier implements IAPIActionApplier {
    ...
    protected final ITVLATabulatorServices engine;
    ...
    public ActionApplier(
        ...
        boolean doCoerceAfterUpdate,
        boolean doBlur,
        boolean freezeStructuresWithMessages, boolean breakIfCoerceAfterUpdateFailed) {
    ...
    public Collection apply(
      ...
      if (AnalysisStatus.debug) 
        nodeLabel = " " + engine.getCurrentLocation();
      ...
      for (Iterator focusIt = focusResult.iterator(); focusIt.hasNext(); ) {
        ...
        for (Iterator assignIt = assigns.iterator(); assignIt.hasNext(); ) {
          ...
          // Compute the values of the let formuale predicates
          HighLevelTVS afterLet;
          if (doLet && action.hasLet()) {
            afterLet = action.evaluateLet(focusedStructure, assign);
            
            if (AnalysisStatus.debug)
              printStructure(
                  afterLet, marker + " After Let " + 
                  nodeLabel + " " + action + 
                  (assign.isEmpty() ? "{}" : " " + assign));                                
          }
          else 
            afterLet = focusedStructure;
          
          // Update formulae evaluation
          status.startTimer(AnalysisStatus.UPDATE_TIME);
          final HighLevelTVS result = action.evaluate(afterLet, assign);
          ...
          if (doLet && action.hasLet()) {
            action.clearLet(result);
            if (AnalysisStatus.debug)
              printStructure(
                  result, marker + " After clearing let" + 
                  nodeLabel + " " + action );     
          }
          ...
          result.filterNodes(new Filter<Node>() {
              public boolean accepts(Node node) {
                  return result.eval(voc.getTVLAKill(), node) != Kleene.trueKleene;
              }
            });
          ...
          if (doCoerceAfterUpdate) {
            ...
            if (!valid) {
              ...
              if (breakIfCoerceAfterUpdateFailed) {
                TVLAAPILog.println("");
                TVLAAPILog.println(StringUtils.newLine + marker +
                    " The analysis has stopped since a constraint was breached " +
                    " during the operation of Coerce, after Update was applied!");
                TVLAAPILog.println("Action = " + action.toString());
                TVLAAPILog.println("Program location = " + nodeLabel);
                TVLAAPILog.println(" input TVS" + structure);
                TVLAAPILog.println(" updated TVS" + result);
                
                
                TVLAAPILog.println("// Input  TVS");
                String structureDT = StructureToDOT.defaultInstance.convert(structure);
                TVLAAPILog.println(structureDT);
                
                TVLAAPILog.println("// Updated TVS");
                String resultDT = StructureToDOT.defaultInstance.convert(result);
                TVLAAPILog.println(resultDT);
    ...
    private void printStructure(HighLevelTVS structure, String header) {
      TVLAAPITrace.tracePrintln("APIApplier: " + header + " TVS = " + structure);
      IOFacade.instance().printStructure(structure, header); 
    }
